...
module.exports = async function handler(req, res) {
  ...
  try {
    const { contract: contractAddress } = req.query;
    ...
    if (!contractAddress) {
      return res.status(400).json({ error: 'contract parameter is required' });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Invalid sentiment data format' });
    }
    ...
    return res.status(200).json(sentimentAnalysis);
